...
export const FindFarmersPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-black/40 backdrop-blur-xl p-6 rounded-xl border border-white/10 shadow-md space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-blue-600/30 border border-blue-500/40 text-blue-300 hover:text-white hover:bg-blue-600 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shrink-0"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {filteredCrops.map((c) => (
          <div
            key={c.id}
            className="p-6 space-y-4 h-full flex flex-col justify-between bg-black/60 backdrop-blur-xl border border-white/10 rounded-xl shadow-xl hover:border-emerald-500/30 transition-all group"
          >
            <div className="flex items-center gap-3 pb-3 border-b border-white/10">
              <img src={c.imageUrl || '/crops/tomato.jpg'} alt={c.name} className="w-14 h-14 rounded-xl object-cover border border-white/10 bg-white/5" />
              <div>
            ...
            <div className="text-xs space-y-1.5 text-gray-400 bg-white/5 p-3 rounded-xl border border-white/5">
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
