...
export const StorageProcessingPage: React.FC = () => {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case 'WHOLESALER': return { bg: 'bg-blue-600/20', text: 'text-blue-300', border: 'border-blue-500/30', icon: <Briefcase className="w-3 h-3" /> };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Header Banner */}
      <div className="bg-black/40 backdrop-blur-xl p-6 md:p-8 rounded-xl border border-white/10 shadow-md">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-xl bg-indigo-600 flex items-center justify-center text-white shadow-md shadow-indigo-950/50">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-600/20 text-indigo-300 border border-indigo-500/30">
          ...
          <div className="flex items-center gap-3">
            <div className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-right">
              <span className="text-xs text-neutral-400 block">Total Storages</span>
              <span className="text-sm font-bold text-emerald-400 font-mono">{storageFacilities.length} Units</span>
            </div>
            <div className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-right">
              <span className="text-xs text-neutral-400 block">Processing Plants</span>
              <span className="text-sm font-bold text-blue-400 font-mono">{processingFacilities.length} Plants</span>
            </div>
            <div className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-right">
              <span className="text-xs text-neutral-400 block">People Directory</span>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === 'STORAGE'
                  ? 'bg-emerald-600 text-white shadow-md shadow-md border border-emerald-500/40'
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === 'PROCESSING'
                  ? 'bg-blue-600 text-white shadow-md shadow-md border border-blue-500/40'
                  : 'bg-white/5 text-neutral-400 hover:text-white hover:bg-white/10 border border-white/10'
              }`}
            >
      ...
      {/* Success Alert */}
      {bookingSuccess && (
        <div className="p-4 bg-emerald-950/80 border border-emerald-500/40 rounded-xl text-xs font-bold text-emerald-300 flex items-center gap-3 shadow-xl">
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${badge.bg} ${badge.text} border ${badge.border}`}>
                            {person.role}
                          </span>
                        </div>
                      ...
                      <div className="bg-white/5 rounded-xl border border-white/10 p-4">
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          {Object.entries(person.extra).map(([key, value]) => (
                            <div key={key}>
                              <span className="text-xs text-neutral-400 uppercase tracking-wider block font-bold">{key}</span>
                              <span className="text-xs text-white font-semibold mt-0.5 block">{String(value)}</span>
                            </div>
                          ))}
                          {person.crops.length > 0 && (
                            <div className="col-span-2 sm:col-span-3">
                              <span className="text-xs text-neutral-400 uppercase tracking-wider block font-bold mb-1">Crops</span>
                              <div className="flex flex-wrap gap-1">
                                {person.crops.map((c, i) => (
                                  <span key={i} className="px-2 py-0.5 rounded-md bg-white/10 text-neutral-200 text-xs font-medium">
                                    🌾 {c}
                                  </span>
                                ))}
                              </div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {person.phone && (
                          <a
                            ...
                            className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-sm shadow-md"
                          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          // Skeleton Cards
          [...Array(6)].map((_, i) => (
            <div key={i} className="bg-black/40 backdrop-blur-xl p-6 rounded-xl border border-white/10 shadow-md space-y-5 animate-pulse h-80">
              ...
              <div className="bg-white/5 h-16 rounded-xl" />
              <div className="flex gap-2">
          ...
          filteredStorage.map((f) => {
            ...
            return (
              <div
                key={f.id}
                className="bg-black/40 backdrop-blur-xl p-6 rounded-xl border border-white/10 shadow-md space-y-5 flex flex-col justify-between hover:border-emerald-500/30 transition-all group"
              >
                <div className="space-y-3.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-mono font-bold text-emerald-400">
                      ₹{f.pricePerTonDay} <span className="text-xs text-neutral-400 font-sans font-normal">/ Ton / Day</span>
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {f.features.map((feat, i) => (
                      <span
                        key={i}
                        className="text-xs px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-neutral-300"
                      >
                        ✓ {feat}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
                  ...
                  <button
                    ...
                    className="py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold text-center border border-emerald-500/40 transition-colors cursor-pointer shadow-sm shadow-md"
                  >
          ...
          filteredProcessing.map((p) => (
            <div
              key={p.id}
              className="bg-black/40 backdrop-blur-xl p-6 rounded-xl border border-white/10 shadow-md space-y-5 flex flex-col justify-between hover:border-blue-500/30 transition-all group"
            >
              <div className="space-y-3.5">
                <div className="flex items-center justify-between">
                  <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-blue-600/20 text-blue-300 border border-blue-500/30">
                ...
                <div className="space-y-2 bg-white/5 p-3.5 rounded-xl border border-white/10 text-xs">
                  <div>
                  ...
                  <div className="pt-1 border-t border-white/5">
                    ...
                    <span className="text-neutral-300 text-xs block mt-0.5">
                ...
                <div className="text-xs text-neutral-400">
                  ...
                  <strong className="text-blue-300 font-mono">{p.minBatchTonnes} Tonnes</strong>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
                ...
                <button
                  ...
                  className="py-2.5 px-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold text-center border border-blue-500/40 transition-colors cursor-pointer shadow-sm shadow-md"
                >
