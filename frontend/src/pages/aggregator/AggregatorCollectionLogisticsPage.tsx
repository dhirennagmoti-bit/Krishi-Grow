    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-black/60 backdrop-blur-2xl border border-white/10 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-cyan-950 transition-all cursor-pointer shrink-0"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {collectionRequests.map((req) => (
          <div
            key={req.id}
            className="p-6 rounded-3xl bg-black/60 backdrop-blur-xl border border-white/10 hover:border-cyan-500/30 transition-all flex flex-col justify-between space-y-5 shadow-xl group"
          >
            <div className="space-y-4">
              ...
              <div className="p-3 rounded-2xl bg-cyan-950/20 border border-cyan-500/20 space-y-1.5 text-xs font-mono">
                ...
                <div className="text-[10px] text-amber-300 pt-1 border-t border-white/10 font-sans flex items-center gap-1">
                  <Clock className="w-3 h-3" /> ETA / Status: {req.eta}
                </div>
              </div>

              {req.notes && (
                <p className="text-[11px] text-neutral-400 italic bg-black/40 p-2.5 rounded-xl border border-white/5">
                  "{req.notes}"
                </p>
              )}
            ...
            <div className="pt-3 border-t border-white/10 flex items-center gap-2">
              {req.status === 'SCHEDULED' && (
                <button
                  ...
                  className="w-full py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-lg"
                >
              ...
              {req.status === 'IN_TRANSIT' && (
                <button
                  ...
                  className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-emerald-950"
                >
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <form onSubmit={handleCreate} className="bg-[#121214] border border-white/15 rounded-3xl max-w-lg w-full p-6 md:p-8 space-y-5 shadow-2xl">
      ...
      {selectedRequestForConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <form onSubmit={handleConfirmCollection} className="bg-[#121214] border border-white/15 rounded-3xl max-w-md w-full p-6 md:p-8 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <h3 className="font-bold text-white text-base flex items-center gap-2">
